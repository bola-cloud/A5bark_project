...
class MotionController extends Controller
{
    ...
    public function index(Request $request) {
        ...
        if ($request->ajax()) {
            ...
            $datatableModel = Datatables::of($model)
                ->addColumn('activation', function ($row_object) use ($permissions) {
                    return view('admin.motion.incs._active', compact('row_object', 'permissions'));
                })
                ->addColumn('actions', function ($row_object) use ($permissions) {
                    return view('admin.motion.incs._actions', compact('row_object', 'permissions'));
                })
        ...
        return view('admin.motion.index', compact('lang','permissions'));
    }
    ...
    public function store(Request $request) {
        ...
        try {
            ...
            if ($request->hasFile('image')) {
                $data['image'] = $request->file('image')->store('motion_images', 'media');
            }
    ...
    private function updateNews(Request $request, Motion $news) {
        $validator = Validator::make($request->all(), [
            'ar_title' => 'required|max:255',
            'en_title' => 'required|max:255',
            'image' => 'nullable',
    ...
    private function activateNews(Motion $news) {
        // Deactivate any currently active festival
        Motion::where('is_active', 1)->update(['is_active' => 0]);
