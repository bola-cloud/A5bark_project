...
class AdverticementController extends Controller
{
    ...
    public function __construct() {
        $this->targetModel = new Adverticement;
    }
    ...
    public function index(Request $request) {
        ...
        if ($request->ajax()) {
            ...
            $datatableModel = Datatables::of($model)
                ->addColumn('activation', function ($row_object) use ($permissions) {
                    return view('admin.adverticement.incs._active', compact('row_object', 'permissions'));
                })
                ->addColumn('actions', function ($row_object) use ($permissions) {
                    return view('admin.adverticement.incs._actions', compact('row_object', 'permissions'));
                })
        ...
        return view('admin.adverticement.index', compact('lang','permissions'));
    }
    ...
    public function store(Request $request) {
        ...
        try {
            ...
            if ($request->hasFile('image')) {
                $data['image'] = $request->file('image')->store('adverticement_images', 'media');
            }
    ...
    private function updateNews(Request $request, Adverticement $news) {
        $validator = Validator::make($request->all(), [
            'ar_head' => 'required|max:255',
            'ar_title' => 'required|max:255',
            'ar_content' => 'required',
    ...
    private function activateNews(Adverticement $news) {
        Adverticement::where('is_active', 1)->update(['is_active' => 0]);
