...
class CourseCategoryController extends Controller
{
    ...
    public function __construct () {
        $this->targetModel = new CourseCategory;
    }
    ...
    public function index (Request $request) {
        ...
        if ($request->ajax()) {
            ...
            ->withCount(['courses', 'trainers', 'students'])
            ->adminFilter();
            
            $datatableModel = Datatables::of($model)
            ->addColumn('activation', function ($row_object) use ($permissions) {
                return view('admin.course_category.incs._active', compact('row_object', 'permissions'));
            })
            ->addColumn('actions', function ($row_object) use ($permissions) {
                return view('admin.course_category.incs._actions', compact('row_object', 'permissions'));
            });

            return $datatableModel->make(true);
        }
        
        return view('admin.course_category.index', compact('lang', 'permissions'));
    }

    public function store (Request $request) {
        $validator = Validator::make($request->all(), [
            'ar_name'     => 'required|max:255|unique:course_categories,ar_name',
            'en_name'     => 'required|max:255|unique:course_categories,en_name',
        ], [
        ...
        return $this->responseTemplate($category, true, __('course_category.object_created'));
    }
    ...
    public function update (Request $request, $id) {
        ...
        return isset($request->activate_object)
            ? $this->activateCourseCategory($category)
            : $this->updateCourseCategory($request, $category);
    }
    ...
    // START HELPERS
    private function updateCourseCategory (Request $request, CourseCategory $category) {

        $validator = Validator::make($request->all(), [
            'ar_name'     => 'required|max:255|unique:course_categories,ar_name,' . $category->id,
            'en_name'     => 'required|max:255|unique:course_categories,en_name,' . $category->id,
        ], [
            'ar_name.required' => __('course_category.ar_name_required'),
            'en_name.required' => __('course_category.en_name_required'),
            'ar_name.max'      => __('course_category.ar_name_max'),
    ...
    private function activateCourseCategory (CourseCategory $category) {
